...
class App extends Component {
  ...
  getImageUrl = () => {
    ...
    const found = imagesList.find(each => each.id === imageId)
  ...
  getActiveTabId = () => {
    ...
    const filteredApp = imagesList.filter(
  ...
  updateTime = () => {
    ...
    const findingSeconds = timeInSeconds <= 0
    if (findingSeconds) {
      this.clearTimerInterval()
  ...
  renderSeconds = () => {
    const {timeInSeconds, isTimeRunning} = this.state
    const findingSeconds = timeInSeconds <= 0
    if (findingSeconds) {
  ...
  resetGame = () => {
    this.setState({score: 0, isTimeRunning: true, timeInSeconds: 60})
  }
  ...
  scoreBoard = () => {
    ...
    return (
      <div className="bg-score">
        ...
        <button type="button" className="but" onClick={this.resetGame}>
          <img
  ...
  renderTrueBoard = () => {
    const {activeTabId} = this.state
    const filteredApp = this.getActiveTabId()

    return (
      <>
        {this.renderSeconds()}
        <img src={this.getImageUrl()} alt="match" className="match-image" />
        ...
        <ul className="unordered">
          {filteredApp.map(eachImage => (
            <AppItem
              imageDetails={eachImage}
              key={eachImage.id}
              setIdThumbnail={this.setIdThumbnail}
            />
          ))}
        </ul>
      </>
  ...
  render() {
    const {isTimeRunning, score, timeInSeconds} = this.state
          ...
          <div className="row-container">
            ...
            <div className="row-container">
              <img
                ...
                className="icon"
              />
              <p className="yellow">{timeInSeconds} sec</p>
            </div>
          </div>
        </div>
        <div className="bg-container">
          {isTimeRunning ? this.renderTrueBoard() : this.scoreBoard()}
        </div>
      </>
    )
  }
